...
fun GameSnapshot.toGameEntity(): GameEntity =
    GameEntity(
        id = gameId,
...
fun GameSnapshot.toGamePlayerEntities(): List<GamePlayerEntity> =
    config.players.mapIndexed { index, player ->
        GamePlayerEntity(
            gameId = gameId,
            playerId = player.id,
            seatOrder = index,
...
fun GameSnapshot.toRoundEntities(): List<RoundEntity> =
    rounds.map { outcome ->
        RoundEntity(
...
fun GameSnapshot.toRoundScoreEntities(): List<RoundScoreEntity> =
    rounds.flatMap { outcome ->
...
fun GameSnapshot.toAttemptEntities(): List<AttemptEntity> =
    rounds.flatMap { outcome ->
        val roundId = "$gameId-round-${outcome.roundNumber}"
