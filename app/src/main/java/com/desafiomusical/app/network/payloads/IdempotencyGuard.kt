...
class IdempotencyGuard(
    ...
    @Synchronized
    fun isNew(eventId: String, timestamp: Long, now: Long = System.currentTimeMillis()): Boolean {
        evictExpired(now)
    ...
    fun isNew(event: GameEvent, now: Long = System.currentTimeMillis()): Boolean =
        isNew(event.eventId, event.timestamp, now)
