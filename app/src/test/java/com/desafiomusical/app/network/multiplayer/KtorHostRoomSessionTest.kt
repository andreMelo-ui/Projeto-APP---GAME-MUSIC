...
class KtorHostRoomSessionTest {
    ...
    private fun newHttpClient(): HttpClient =
        HttpClient(ClientCIO) { install(ClientWebSockets) }.also { httpClient = it }

    private fun joinEvent(playerId: String, roomCode: String) = GameEvent.JoinRoom(
    ...
    fun `broadcast chega em todos, sendTo isolado, eventos duplicados sao ignorados e desconexao atualiza jogadores`(): Unit =
        runBlocking {
            ...
            val readyOnlyForP1 = GameEvent.PlayerReady(
                eventId = UUID.randomUUID().toString(),
                timestamp = System.currentTimeMillis(),
                playerId = "p1",
                ready = true
    ...
    @Test
    fun `roomCode invalido no JoinRoom faz o host fechar a conexao`(): Unit = runBlocking {
    ...
    @Test
    fun `reconexao com o mesmo playerId substitui a conexao antiga sem derrubar o roster`(): Unit = runBlocking {
        val roomCode = "RECN1"
        val session = KtorHostRoomSession(roomCode = roomCode).also { host = it }
        session.start()
        ...
        val readyEvent = GameEvent.PlayerReady(
            eventId = UUID.randomUUID().toString(),
            timestamp = System.currentTimeMillis(),
            playerId = "p1",
            ready = true
